...
1. **세션 이탈 = 폐기**: 정상 흐름은 반드시 평가까지 마치는 것이지만, 각 단계에서 강제로 빠져나올 수 있습니다. PBL "종료"와 대화 시뮬레이션 좌측 하단 "종료" 버튼은 모두 확인 모달을 거쳐 **평가 없이 세션을 폐기**하고 시나리오 목록으로 나갑니다(백엔드 호출 없이 이탈, 세션은 해당 단계로 서버에 남음). 대화 시뮬레이션 헤더의 "완료 및 평가하기"는 이와 구분되는 정상 종결(대화 종료 → 평가 진행) 액션입니다.
